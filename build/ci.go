...
import (
	...
	"github.com/CrestChain/go-crest/crypto/signify"
	"github.com/CrestChain/go-crest/internal/build"
	"github.com/CrestChain/go-crest/params"
	"github.com/cespare/cp"
)

var (
	// Files that end up in the gcrest*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gcrest"),
	}

	// Files that end up in the gcrest-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Crestchain contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "Crestchain bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (Crestchain Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "gcrest",
			Description: "Crestchain CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "Crestchain private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Crestchain account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gcrest     = "gcrest-" + basegeth + ext
		alltools = "gcrest-alltools-" + basegeth + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gcrest, gethArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gcrest, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gcrest-release.pub"
...
func doDocker(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, gcrestonly
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, gcrestonly
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, gcrest& tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, gcrest& tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, gcrestonly
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, gcrest& tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, gcrestonly
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, gcrest& tools
	var tags []string
	...
	if len(*manifest) != 0 {
		...
		// Assemble and push the gcrestmanifest image
		for _, tag := range tags {
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gcrest-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gcrest-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "Crestchain Builds <fjl@ethereum.org>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gcrest.exe" {
			gethTool = file
		} else {
	...
	// first section contains the gcrest binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Geth":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gcrest.nsi", filepath.Join(*workdir, "gcrest.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gcrest-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gcrest.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/CrestChain/go-crest/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gcrest.aar", filepath.Join(GOBIN, "gcrest.aar"))
		os.Rename("gcrest-sources.jar", filepath.Join(GOBIN, "gcrest-sources.jar"))
		return
	}
	...
	archive := "gcrest-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gcrest.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/CrestChain/go-crest/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "gcrest-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
