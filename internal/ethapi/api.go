...
// PublicEthereumAPI provides an API to access Crestchain related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicEthereumAPI struct {
	b Backend
}

// NewPublicEthereumAPI creates a new Crestchain protocol API.
...
// Sign calculates an Crestchain ECDSA signature for:
...
//
// https://github.com/CrestChain/go-crest/wiki/Management-APIs#personal_sign
...
//
// https://github.com/CrestChain/go-crest/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Crestchain signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gcrest.
...
// PublicBlockChainAPI provides an API to access the Crestchain blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Crestchain blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current crestchain chain config.
...
// PublicDebugAPI is the collection of Crestchain APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Crestchain APIs exposed over the private
// debugging endpoint.
...
// Version returns the current crestchain protocol version.
