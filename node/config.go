...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gcrest is "gcrest". If no
...
func (c *Config) NodeName() string {
	...
	if name == "gcrest" || name == "gcrest-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gcrest" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gcrest 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gcrest' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
