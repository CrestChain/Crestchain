...
// Package gethclient provides an RPC client for gcrest-specific APIs.
package gethclient
...
// Client is a wrapper around rpc.Client that implements gcrest-specific functionality.
//
// If you want to use the standardized Crestchain RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
// GCStats retrieves the current garbage collection stats from a gcrest node.
...
// MemStats retrieves the current memory stats from a gcrest node.
...
// GetNodeInfo retrieves the node info of a gcrest node.
