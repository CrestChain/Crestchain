...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.crestchain", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gcrest-android-")
	if err != nil {
	...
	// Generate the mobile bindings for gcrestand add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/CrestChain/go-crest/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gcrest.aar"), "gcrest.aar")
