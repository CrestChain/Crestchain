...
import (
	...
	"github.com/CrestChain/go-crest/accounts/keystore"
	"github.com/CrestChain/go-crest/common"
	"github.com/CrestChain/go-crest/common/fdlimit"
	"github.com/CrestChain/go-crest/consensus"
	"github.com/CrestChain/go-crest/consensus/clique"
	"github.com/CrestChain/go-crest/consensus/ethash"
	...
	"github.com/CrestChain/go-crest/eth"
	...
	"github.com/CrestChain/go-crest/eth/tracers"
	"github.com/CrestChain/go-crest/ethdb"
	"github.com/CrestChain/go-crest/ethstats"
	"github.com/CrestChain/go-crest/graphql"
	"github.com/CrestChain/go-crest/internal/ethapi"
	"github.com/CrestChain/go-crest/internal/flags"
	"github.com/CrestChain/go-crest/les"
	"github.com/CrestChain/go-crest/log"
	"github.com/CrestChain/go-crest/metrics"
	"github.com/CrestChain/go-crest/metrics/exp"
	"github.com/CrestChain/go-crest/metrics/influxdb"
	...
	"github.com/CrestChain/go-crest/p2p/nat"
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		if ctx.GlobalBool(RopstenFlag.Name) {
			// Maintain compatibility with older gcrestconfigurations storing the
...
// for gcrestand returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gcrest account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Crestchain client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Crestchain) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Crestchain service: %v", err)
		}
...
// RegisterEthStatsService configures the Crestchain Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Crestchain Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gcrest.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.GlobalString(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gcrest.", tagsMap)
		}
...
// new format.
//
// e.g. gcrest account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gcrest --keystore /tmp/mykeystore --lightkdf account new
//
