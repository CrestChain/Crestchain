...
This setup allows a DApp to connect to a remote Crestchain node and send transactions that are locally signed. This can help in situations when a DApp is connected to an untrusted remote Crestchain node, because a local one is not available, not synchronised with the chain, or is a node that has no built-in (or limited) account management.
...
GLOBAL OPTIONS:
   ...
   --keystore value        Directory for the keystore (default: "$HOME/.crestchain/keystore")
...
The general flow for signing a transaction using e.g. gcrestis as follows:
![image](sign_flow.png)

In this case, `gcrest` would be started with `--signer http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
* gcresttodos
    ...
    - Relay: gcrestshould be started in `gcrest --signer localhost:8550`.
    - Currently, the gcrestAPIs use `common.Address` in the arguments to transaction submission (e.g `to` field). This type is 20 `bytes`, and is incapable of carrying checksum information. The signer uses `common.MixedcaseAddress`, which retains the original input.
    - The gcrestAPI should switch to use the same type, and relay `to`-account verbatim to the external API.
* [x] Storage
  ...
  "params": [
    {
      "accounts": [
        {
          "url": "keystore:///home/bazonk/.crestchain/keystore/UTC--2017-11-20T14-44-54.089682944Z--123409812340981234098123409812deadbeef42",
          "address": "0x123409812340981234098123409812deadbeef42"
        },
        {
          "url": "keystore:///home/bazonk/.crestchain/keystore/UTC--2017-11-23T21-59-03.199240693Z--cafebabedeadbeef34098123409812deadbeef42",
...
* A Graphical UI MUST show the blocky-identicon for crestchain addresses.
