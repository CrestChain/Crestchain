...
func TestAccountListEmpty(t *testing.T) {
	gcrest := runGeth(t, "account", "list")
	gcrest.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gcrest := runGeth(t, "account", "list", "--datadir", datadir)
	defer gcrest.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gcrest := runGeth(t, "account", "new", "--lightkdf")
	defer gcrest.ExpectExit()
	gcrest.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gcrest := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gcrest.ExpectExit()
	gcrest.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gcrest := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gcrest := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gcrest.ExpectExit()
	gcrest.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gcrest.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gcrest := runMinimalGeth(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gcrest.StderrText(), m) {
...
// https://github.com/CrestChain/go-crest/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gcrest.SetTemplateFunc("keypath", func(file string) string {
