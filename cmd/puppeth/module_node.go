...
// nodeDockerfile is the Dockerfile required to run an Crestchain node.
var nodeDockerfile = `
...
{{end}}
RUN \
  echo 'gcrest --cache 512 init /genesis.json' > gcrest.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.crestchain/keystore/ && cp /signer.json /root/.crestchain/keystore/' >> gcrest.sh && \{{end}}
	echo $'exec gcrest --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gcrest.sh

ENTRYPOINT ["/bin/sh", "gcrest.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Crestchain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.crestchain{{if .Ethashdir}}
...
// deployNode deploys a new Crestchain node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gcrest --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.crestchain"],
