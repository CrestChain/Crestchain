...
Use `devp2p enrdump <base64>` to verify and display an Crestchain Node Record.
...
Run `devp2p key to-enode mynode.key -ip 127.0.0.1 -tcp 30307` to create an enode:// URL
...
1. initialize the gcrest node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gcrest with the following flags:
```
gcrest --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gcrest node:
 ```
...
To run the eth66 protocol test suite, initialize a gcrest node as described above and run the following command,
replacing `<enode>` with the enode of the gcrest node:
...
[dns-tutorial]: https://gcrestcrestchain.io/docs/developers/dns-discovery-setup
