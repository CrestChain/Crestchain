...
```shell
go get github.com/CrestChain/go-crest/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://gcrestcrestchain.io/docs/clef/tutorial) .
...
*It is worth noting that the connected gcrestnode can be a fullnode or a light client. If it is fullnode, you must enable the LES protocol. E.G. add `--light.serv 50` to the startup command line flags*.
...
*CHECKPOINT_HASH is obtained based on this [calculation method](https://github.com/CrestChain/go-crest/blob/master/params/config.go#L251).*
...
* Start gcrest with the modified configuration file
